...
def get_channels(dataset, measurement_type=ANY):
    """ Get a list of `SubChannel` instances from a `Dataset` by their
        measurement type(s).
    ...
    return filter_channel(dataset.getPlots(filter=False), measurement_type)
