...
def test_rolling_fft():
    ...
    # Get the Individual FFTs
    first_fft = fft.aggregate_fft(df_accel[:1.0])
    second_fft = fft.aggregate_fft(df_accel[1.0:])

    # Do Assertions
    npt.assert_almost_equal(
        df_rolling_fft['value'].iloc[:501].to_numpy(),
        first_fft['A'].to_numpy())
    npt.assert_almost_equal(
        df_rolling_fft['value'].iloc[-501:].to_numpy(),
        second_fft['A'].to_numpy())
